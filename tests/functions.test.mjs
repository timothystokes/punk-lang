...
test('named function — calling with structured arg', () => {
  assert.equal(
    punk('welcome:([name]){Hello name?}  welcome!{Tim}'),
    '{Hello Tim}'
  );
});
...
test('simple nested function call — body returns a singleton template', () => {
  ...
  assert.equal(
    punk('circ:([r]){X!{X!{3.141 r?} 2}}  circ!7'),
    '{43.974}'
  );
});
