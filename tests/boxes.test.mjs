...
test('write then read a box', () => {
  ...
  // box and calling a 1-slot fn produces `{{42}}` — the body has one
...
test('a box can hold any value, including a template', () => {
  // The box stores the stage's running value (here `{a b c}`).
...
test('boxes and ordinary names do not interact', () => {
  ...
  // The box read goes through a 1-slot fn, picking up a uniform wrap.
  assert.equal(
